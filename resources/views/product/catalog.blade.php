...
<div class="row" style="margin-top:50px">
    <h2><b>Catalogo de productos</b></h2>
</div>
  <ul class="prodlist">
  @foreach($products as $product)
    <li class="one_third">
        <div class="thumb"><a href=""><img src="{{ asset('products/'.$product->imagen) }}" alt="" /></a></div>
        <div class="content">
            <div class="contentinner">
                <div>
                    <span class="price">BsF: {{$product->monto}}</span>
                    <a href="" class="title">{{$product->name}}</a>
                </div>
                <p class="desc">fecha: {{$product->fecha}}</p>
                <p>{!!Html::linkRoute('product.showClient',$title = '', $parameters = [$product->id], $attributes = ['class'=> 'glyphicon glyphicon-search btn btn-success'])!!}</p>
            </div>
        </div>
    </li>
  @endforeach 
